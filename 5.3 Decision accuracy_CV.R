...
dat.CV$agreed <- as.numeric((dat.CV$tr==1)==(dat.CV$benefit.survival.at.fixed.time>0))
table(dat.CV$agreed)
table(dat.CV$tr==0, dat.CV$benefit.survival.at.fixed.time>0)
km23 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$agreed==0,]) ## group 2U3, see main paper
km14 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$agreed==1,]) ## group 1U4, see main paper
km24 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$tr==0,]) ## group 2U4, see main paper
km13 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$tr==1,]) ## group 1U3, see main paper
km1 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$tr==1&dat.CV$benefit.survival.at.fixed.time>0,]) ## group 1, see main paper
...
p.neg=mean(dat.CV$benefit.survival.at.fixed.time<0)
v.p.neg=p.neg*(1-p.neg)/N

km3 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$tr==1&dat.CV$benefit.survival.at.fixed.time<0,]) ## group 3, see main paper
km4 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$tr==0&dat.CV$benefit.survival.at.fixed.time<0,]) ## group 4, see main paper
...
PB.0 <- summary(km14, times=t.0)[6][[1]]-summary(km24, times=t.0)[6][[1]]
SE.PB.0 <- sqrt(summary(km14, times=t.0)[7][[1]]^2+summary(km24, times=t.0)[7][[1]]^2+summary(km1, times=t.0)[7][[1]]^2)
...
v.p.pos=v.p.neg
km1 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$tr==1&dat.CV$benefit.survival.at.fixed.time>0,]) ## group 1, see main paper
km2 <- survfit(Surv(time, status)~1, data=dat.CV[dat.CV$tr==0&dat.CV$benefit.survival.at.fixed.time>0,]) ## group 4, see main paper
...
rmst_treatment <- with(dat.CV,rmst2(time, status, tr,tau=t.star))
rmst_model <- with(dat.CV,rmst2(time, status, agreed,tau=t.star))
...
# estimate treatment effect in subpopulation of patients with negative predicted benefit
with(dat.CV[dat.CV$benefit.survival.at.fixed.time<0,],rmst2(time, status, tr,tau=t.star))



## population benefit for median survival -----
mtr=survfit(Surv(time, status) ~ tr, data = dat.CV, type = "kaplan-meier")
m.model=survfit(Surv(time, status) ~ agreed, data = dat.CV, type = "kaplan-meier")

#PB 1
m14=survfit(Surv(time, status) ~ 1, data = dat.CV[dat.CV$agreed==1,], type = "kaplan-meier") # Group 1U4
m13=survfit(Surv(time, status) ~ 1, data = dat.CV[dat.CV$tr==1,], type = "kaplan-meier") # Group 1U3
summary(m14)$table[7]-summary(m13)$table[7]

#Treatment effect in patients with negative benefit 
m34=survfit(Surv(time, status) ~ tr, data = dat.CV[dat.CV$benefit.survival.at.fixed.time<0,], type = "kaplan-meier") # Group 1U4
