...
ggplot(data = dt2) + 
  ...
  scale_size_manual(values=c(0.6,0.4,0.6,0.4))+ 
...
group1 <- survdat; group1$tr="1"

for(i in 1: N){  group0$wb[i]=prediction.weibull(group0[i,], fit.model)[t.0*100,2] }
survdat$prob.t0.cll=log(-log(group0$wb)) # complimentary log-log transformation

for(i in 1: N){  group1$wb[i]=prediction.weibull(group1[i,], fit.model)[t.0*100,2] }
survdat$prob.t1.cll=log(-log(group1$wb)) # complimentary log-log transformation


calibrate.cox0 <- coxph(Surv(time, status)~ rcs(prob.t0.cll,3),x=T,
                        data=survdat[survdat$tr=="0",])

calibrate.cox1 <- coxph(Surv(time, status)~ rcs(prob.t1.cll,3),x=T,
                        data=survdat[survdat$tr=="1",])

predict.calibrate.cox <- -predictSurvProb(calibrate.cox0,
                                          newdata=data.frame(prob.t0.cll=survdat$prob.t0.cll), 
                                          times=t.0)+
  ( predictSurvProb(calibrate.cox1,newdata=data.frame(prob.t1.cll=survdat$prob.t1.cll), 
                    times=t.0))

dd1=data.frame(predicted=survdat$benefit.survival.at.fixed.time, observed=predict.calibrate.cox) 
